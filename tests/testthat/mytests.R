...
test_that("rm_compactsum works with effSize = TRUE, show.tests = TRUE, but pvalue = FALSE", {
  output <- rm_compactsum(data = pembrolizumab, xvars = c("age", "change_ctdna_group"), grp = "cohort", use_mean = "age", tableOnly = TRUE, digits = 2, pvalue = FALSE, effSize = TRUE, show.tests = TRUE)
...
test_that("rm_compactsum works with sd == 0", {
  output <- rm_compactsum(data = temp, xvars = "age", use_mean = "age", tableOnly = TRUE)
