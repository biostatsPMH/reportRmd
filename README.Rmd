...
Changes to the handling of Dates causes an error when summarising date variables in rm_covsum. Please install the development version.
...
- `rm_mvsum()` and `forestplotMV()` now support `include_unadjusted=TRUE` to display univariate (unadjusted) and multivariable (adjusted) estimates side-by-side
- `forestplotMV()` data parameter is now optional - data extracted automatically from model objects
...
As of v0.1.3 you can now use `xvars` and `grp` as aliases for covs and maincov in `rm_covsum`.
...
Forest plots can display multivariable results, or include univariate estimates for comparison:
