...
The `rm_compactsum` function is similar to the `rm_covsum` function, but condenses the table by excluding the reference rows and missing data rows and creating instead a missing data column. The arguments largely function the same way with the exception that displaying the iqr is the default and the function provides more flexibility. Future versions will align the two functions.
...
Both `rm_covsum` and `rm_compactsum` support tidyselect syntax, allowing you to use
bare column names instead of character strings. This also enables piping with `|>`:

```{r}
pembrolizumab |> rm_compactsum(xvars = c(age, pdl1, change_ctdna_group), grp = sex)
```

The `xvars`/`grp` aliases can also be used with `rm_covsum` for a consistent interface:
...
Header rows can be added to any table using the `header_above` argument in `outTable()`.
This works for HTML, PDF and Word output. The argument takes a named vector where the names
are the header labels and the values are the number of columns each label spans.
...
Outputs from multiple imputations can be summarised as follows (not run):
...
As of version 0.1.3, you can include univariate (unadjusted) estimates alongside the multivariable estimates in a single table by setting `include_unadjusted = TRUE`. This provides a convenient way to compare unadjusted and adjusted estimates side-by-side.

```{r}
glm_fit <- glm(orr~change_ctdna_group+pdl1+age,
               family='binomial',
               data = pembrolizumab)
rm_mvsum(glm_fit, include_unadjusted = TRUE, showN = TRUE, vif = TRUE)
```

The data are taken from the model itself, so the `data` argument is not needed. This includes Cox proportional hazards models, where the response is a `Surv()` object:

```{r}
require(survival)
cox_fit <- coxph(Surv(os_time, os_status) ~ age + sex + baseline_ctdna,
                 data = pembrolizumab)
rm_mvsum(cox_fit, include_unadjusted = TRUE)
```

Transformed terms are summarised on the scale they were fit on, so the
unadjusted estimate for `log(l_size)` below is the slope from
`glm(orr ~ log(l_size))`, not from `glm(orr ~ l_size)`:

```{r}
rm_mvsum(glm(orr ~ log(l_size) + sex, family = 'binomial', data = pembrolizumab),
         include_unadjusted = TRUE)
```

Two points are worth knowing when reading these tables.

The univariate models are fit on the same complete-case sample as the
multivariable model, so the two sets of estimates are directly comparable and
the `N` column applies to both. This differs from calling `rm_uvsum()`
yourself, which uses all rows available for each covariate.

A covariate that appears only inside an interaction term has no adjusted
estimate of its own. Such covariates are appended to the end of the table with
an unadjusted estimate and a blank adjusted column, rather than being dropped:

```{r}
rm_mvsum(glm(orr ~ pdl1 + age:sex, family = 'binomial', data = pembrolizumab),
         include_unadjusted = TRUE)
```

Terms that expand to a matrix, such as `poly(age, 2)`, can not be fit
univariately; these are skipped with a warning and their unadjusted cells are
left blank.
...
## Including Median Follow-up

The reverse Kaplan-Meier follow-up time can also be displayed.

```{r}
rm_survsum(data=pembrolizumab,time='os_time',status='os_status',
 group="sex",survtimes=seq(12,24,12),survtimeunit='months',
 add.reverse.KM = T)
```
...
For competing risk analysis, use `rm_cifsum()` to display cumulative incidence probabilities at specific time points. This requires the cmprsk package and a status variable coded with different event types.
...
`predsum()` summarises how well a set of predicted probabilities recovers a
binary outcome. Supply the observed outcome, the predicted probabilities and
the cut-off used to turn those probabilities into predicted classes.

```{r}
d <- pembrolizumab[complete.cases(pembrolizumab[, c('change_ctdna_group',
                                                    'baseline_ctdna',
                                                    'age', 'l_size')]), ]
fit <- glm(change_ctdna_group ~ baseline_ctdna + age + l_size,
           family = 'binomial', data = d)
d$predicted <- predict(fit, type = 'response')

predsum(true = d$change_ctdna_group, predicted = d$predicted, cutoff = 0.5)
```

The default output is the set of statistics usually reported in a clinical
setting: prevalence, sensitivity, specificity, positive and negative predictive
value, accuracy and AUC. The table also records the sample size, the number of
events, the cut-off used, and the level of the outcome being treated as the
event.

That last row is worth reading. For a factor outcome the second level is taken
as the event, so above the model is predicting *Increase from baseline*. If
that is not what you intended, set `positive` explicitly:

```{r eval=FALSE}
predsum(true = d$change_ctdna_group, predicted = d$predicted, cutoff = 0.5,
        positive = 'Decrease from baseline')
```

## Choosing Which Statistics to Report

The `output` argument selects statistics by name, and accepts several
machine-learning measures that are not shown by default:

```{r}
predsum(true = d$change_ctdna_group, predicted = d$predicted, cutoff = 0.5,
        output = c('accuracy', 'brier', 'f1', 'auc'))
```

Brier score, F1 and AUC-PR have no closed-form confidence interval, so
`predsum()` obtains theirs by bootstrap. This is slower than the other
statistics; `nboot` controls the number of replicates (default 2000).
...
As of version 0.1.3, you can include both unadjusted and adjusted estimates in a multivariable forest plot by setting `include_unadjusted = TRUE`:

```{r, eval=F,echo=T}
glm_fit <- glm(orr~pdl1+age,
               family='binomial',
               data = pembrolizumab)
forestplotMV(glm_fit, include_unadjusted = TRUE)
```
...
glm_fit <- glm(orr~pdl1+age,
               ...
               data = pembrolizumab)
forestplotMV(glm_fit, include_unadjusted = TRUE)
ggsave('images/forestmv_unadj.png', scale = 0.5,height = 1,width =4)
```

![](images/forestmv_unadj.png){width=100%}
...
This function will search the current script or markdown file for R packages and record the package name, functions called, the package version and the citation(s) for the package. By default, function calls and packages that only appear in commented code will be ignored, but they can be included by setting `ignore_comments` = FALSE
...
| Package name | Functions called | Package version | Package citation |
|-------------|------------------|-----------------|------------------|
...
Two parts of the package draw random numbers, and both are seeded by default so
that a report re-knitted next month produces the same table as the one already
circulated.

## Monte Carlo p-values

`rm_covsum()` and `rm_compactsum()` compare categorical variables with a
chi-squared test, switching to Fisher's exact test when any *expected* cell
count falls below five. On a large or sparse table the exact test can exceed its
workspace, in which case both functions fall back to a Monte Carlo
approximation and say so:

```{r eval=FALSE}
Fisher's exact test could not be computed for site; using Monte Carlo
simulation with B = 1000 and seed = 1.
```

`B` sets the number of replicates and `seed` fixes the random state for the
duration of the call; the session's own random state is restored afterwards, so
seeding here does not disturb anything else in your document. Setting
`seed = NULL` restores the older behaviour in which the p-value varies between
calls.

```{r eval=FALSE}
rm_covsum(data = pembrolizumab, maincov = 'cohort', covs = c('age', 'sex'),
          B = 5000, seed = 2024)
```

When a Monte Carlo p-value has been used, `show.tests = TRUE` records the
replicate count alongside the test name, so the table itself documents how the
p-value was obtained.

Because both functions now use the same expected-count rule, `rm_covsum()` and
`rm_compactsum()` select the same test and report the same p-value for the same
data.

## Bootstrap confidence intervals

`predsum()` bootstraps the intervals for Brier score, F1 and AUC-PR. It takes
the same `seed` argument, with the same default and the same restoration
behaviour:

```{r eval=FALSE}
predsum(true = d$change_ctdna_group, predicted = d$predicted, cutoff = 0.5,
        output = c('brier', 'f1'), nboot = 5000, seed = 2024)
```
