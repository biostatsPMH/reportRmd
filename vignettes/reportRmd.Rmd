...
cox_fit <- coxph(Surv(os_time, os_status) ~ age + sex + baseline_ctdna,
                 data = pembrolizumab)
rm_mvsum(cox_fit, include_unadjusted = TRUE, tableOnly = TRUE)
```
...
rm_mvsum(glm(orr ~ log(l_size) + sex, family = 'binomial', data = pembrolizumab),
         include_unadjusted = TRUE, tableOnly = TRUE)
```
...
This provides a convenient alternative to the `forestplotUVMV` function for comparing unadjusted and adjusted estimates.
