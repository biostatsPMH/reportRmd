...
#' Match column types between data frames
#'
#' Converts column types in source to match target for all shared columns.
#' Particularly useful before binding rows.
#'
#' @param source Data frame to modify
#' @param target Data frame with target column types
#'
#' @return Modified source data frame with matched column types
#' @keywords internal
#' @noRd
match_column_types <- function(source, target) {
  ...
  for (col in common_cols) {
    ...
    if (is.factor(target[[col]])) {
      ...
      # For non-factors, just match the class ----
      class(source[[col]]) <- class(target[[col]])
    }
  }
...
#'
#' @param ignore_comments Logical. If TRUE (default), ignores function calls
#'   within commented code (both R comments starting with # and HTML/XML
#'   comments <!-- -->). If FALSE, extracts functions from all code including
#'   commented sections.
#'
