...
#' @importFrom rlang as_string enquo
    ...
    if (is.factor(data[[cov]])) {
      ...
      if (!is.null(maincov)) {
        if (pvalue) {
          ...
          if ((missing_testcat & lowcounts) | testcat == "Fisher") {
            p_type <- "Fisher Exact"
            # NULL sentinel signals the Fisher test failed, triggering MC sim.
            p <- tryCatch(
          ...
          p <- lpvalue(p)
          if (effSize) {
            # Effect size for categorical variables is Cramer's V regardless of
            # which p-value test (Fisher / MC sim / Chi-squared) was used.
            e_type <- "Cramer's V"
      ...
      onetbl <- mapply(
        function(sublevel, N) {
          ...
          sumCov <- tryCatch(
            round(summary(subdata[[cov]]), digits),
            error = function(cnd) NA
          )
...
rm_covsum <- function(
  ...
  # Idiom is consistent across rm_uvsum/rm_mvsum/rm_compactsum: variables are
  # captured with rlang::enquo() (bare, imported via NAMESPACE) and resolved
  # with tidyselect::eval_select(). The substitute()/is.null() guards below
  # handle the covs-or-xvars (and maincov-or-grp) aliases and the case where an
  # argument is supplied explicitly as NULL.
  ...
  # NOTE: arguments are forwarded to covsum() by matching the caller's supplied
  # argument names against covsum()'s formals. This is a deliberate but implicit
  # coupling: renaming or removing a covsum() formal silently changes what
  # rm_covsum forwards (no error is raised). Keep covsum()'s argument names in
  # sync with the pass-through arguments documented for rm_covsum.
...
rm_survdiff <- function(
  ...
  formatp <- resolve_formatp(unformattedp)
