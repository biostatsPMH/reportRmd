...
forestplotMV <- function(model, data = NULL, include_unadjusted = FALSE,
  ...
  if (is.na(x_lab)) {
    stop("forest plots are only available for models reporting odds ratios or relative risks.")
  }
  if (!include_unadjusted) x_lab <- paste("Adjusted", x_lab)
  ...
  if (include_unadjusted) {
    ...
    uv_mods <- rm_uvsum(response = ma$response, covs = ma$predictors,
                        data = uv_data, returnModels = TRUE)
    uv_tabs <- lapply(uv_mods, function(mod) {
      m_summary(mod, CIwidth = conf.level, digits = digits,
                whichp = "levels", for_plot = TRUE)
    })
    tab_uv <- do.call(rbind, uv_tabs)
    tab_uv <- prepare_forest_data(tab_uv, model_type = "Unadjusted", digits = digits)
...
deduplicate_refs <- function(tab) {
  ...
  if (length(ref_rows) == 0) return(tab)
...
#' @keywords internal
build_forest_ggplot <- function(tab, x_lab,
                                show_linetype = FALSE,
                                colours = "default",
                                showEst = TRUE,
                                showRef = TRUE,
