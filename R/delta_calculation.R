...
calc_CramerV <- function(chisq_test) {
  ...
  # this is ugly, but we need to re-create the data
...
#    new_summary <- summary(new_chi)
    chi_toCramer(new_chi) # NOTE: this really should be changed so that negative values are truncated at zero
  }
...
calc_WilcoxonR <- function(wilcox_test) {
  ...
  #  new_stat <- new_wilcox$statistic
    wilcox_effSize(new_wilcox) # NOTE: this really should be changed so that negative values are truncated at zero
  }
...
calc_epsilonSq <- function(kruskal_test) {
  ...
  #  new_stat <- new_kruskal$statistic
    chi_toEpsilonSq(new_kruskal) # NOTE: this really should be changed so that negative values are truncated at zero
  }
