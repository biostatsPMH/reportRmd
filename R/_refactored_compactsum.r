...
#'   confidence intervals included in the table
...
rm_compactsum2 <- function(data,
  ...
  if (missing(xvars)) stop("xvars is a required argument")

  # Process variable selections with tidyselect
  x_vars <- tidyselect::eval_select(
  ...
  grouping_var <- NULL
  if (!missing(grp)) {
    grouping_var <- tidyselect::eval_select(
    ...
    if (length(grouping_var) > 1) {
      stop("Only one grouping variable is allowed")
    }
  }

  # Override use_mean if all.stats is TRUE
  if (all.stats) {
    use_mean <- FALSE
  }
  ...
  if (length(missing_vars) > 0) {
    stop(paste0("These xvars are not in the data: '",
  ...
  if (length(all_miss) > 0) {
    warning(paste("The following variables have only missing data and will be omitted from the output:",
  ...
  if (!missing(grp) & (effSize | show.tests | pvalue)) {
    ...
    if (any(grp_tab < 2)) {
      warning("Small counts in '", grouping_var,
              "'. No statistical tests or effect size will be reported.")
      effSize <- FALSE
      pvalue <- FALSE
      show.tests <- FALSE
    } else if (any(grp_tab < 5)) {
      warning("Small sample size in '", grouping_var,
              "' group may lead to unstable effect sizes.")
    }
  }
    ...
    if (!inherits(use_mean, "logical")) {
      ...
      for (xvar in use_mean) {
        if (!is.numeric(data[[xvar]])) {
          stop(paste0("use_mean can only be specified for numeric columns. Variable ",
                      xvar, " is not numeric."))
        }
      }
    }
  ...
  if (length(digits) > 1) {
    if (!all(names(digits) %in% names(data))) {
      stop(paste("The following variables were specified in digits but are not found in the data:",
                 setdiff(names(digits), names(data))))
    }
  }
  ...
  # Handle missing grouping data
  if (!missing(grp)) {
    n_na <- sum(is.na(data[[grouping_var]]))
    if (n_na > 0) {
      message(paste("There are", n_na, "observations missing", grouping_var,
  ...
  if (dt_msg) {
    message("no statistical tests will be applied to date variables, date variables will be summarised with median")
  }
  ...
  for (xvar in x_vars) {
    ...
    var_type <- assign_method(data, xvar, use_mean)
    class(xvar) <- c(class(xvar), var_type)

    # Get digits for this variable
    dig <- assign_digits(xvar, digits)

    # Call appropriate function
    output_list[[xvar]] <- xvar_function(
      xvar = xvar,
      data = data,
      grp = grouping_var,
      digits = dig,
  ...
  # Remove Missing column if all zeros
  ...
  if ("p-value" %in% colnames(result)) {
    if (!pvalue) {
      ...
      result[["p-value"]] <- p.adjust(result[["p-value"]], method = p.adjust)
      small_p <- which(result$`p-value` < 0.05)
      if (!unformattedp) {
  ...
  bold_cells <- cbind(which(result[, 1] %in% names(data) & result$disp != ""),
                      rep(1, length(which(result[, 1] %in% names(data) & result$disp != ""))))
  ...
  attr(result, "description") <- generate_description(x_vars, output_list)
  if (!missing(grp) && exists("n_na") && n_na > 0) {
    attr(result, "description") <- paste(n_na, "entries were missing data on",
                                         grouping_var, "and were removed prior to analysis.",
  ...
  # Add bold cells for significant p-values
  if ("p-value" %in% colnames(result)) {
  ...
  # Note: outTable is from the reportRmd package
  # If you need to internalize this, replace with your own formatting function
  outTable(result, caption = caption, nicenames = nicenames,
           to_indent = to_indent, bold_cells = bold_cells)
}

#' Create a summary table for an individual covariate (generic)
#' @keywords internal
xvar_function <- function(xvar, data, grp, digits = 1, digits.cat = 0,
                          iqr = TRUE, all.stats = FALSE, pvalue = TRUE, effSize = FALSE,
                          show.tests = FALSE, percentage = "col") {
  UseMethod("xvar_function", xvar)
}

#' Default method for xvar_function
#' @keywords internal
xvar_function.default <- function(xvar, ...) {
  stop("No default method for xvar_function. The xvar class must be known")
}

#' Date variable summarization
#' @keywords internal
xvar_function.rm_date <- function(xvar, data, grp, digits = 1,
                                  digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                  pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                  percentage = "col") {
  # No testing for dates
  pvalue <- FALSE
  effSize <- FALSE
  show.tests <- FALSE
  ...
  if (!missing(grp)) {
    ...
    for (g in levels(group_var)) {
      sub_data <- x_var[group_var == g]
      df[, paste0(g, " (n=", sum(group_var == g), ")")] <- date_sum(sub_data)
    }
  }
...
#' Binary numeric variable summarization
#' @keywords internal
xvar_function.rm_binary <- function(xvar, data, grp, digits = 1,
                                    digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                    pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                    percentage = "col") {
  if (!(pvalue | effSize)) show.tests <- FALSE
  ...
  if (percentage == "row") {
    ...
    df[, paste0("Full Sample (n=", nrow(data), ")")] <-
      paste0(sum(x_var, na.rm = TRUE), " (",
             format(round((100 * sum(x_var, na.rm = TRUE) /
                             (nrow(data) - sum(is.na(x_var)))), digits.cat),
                    nsmall = digits.cat), "%)")
  }
  ...
  if (!missing(grp)) {
    ...
    for (grp_level in grp_levels) {
      ...
      df[1, title] <- binary_xvar_helper(grp_level, data, xvar, grp,
                                         digits.cat, percentage)
    }
...
#' Mean summarization
#' @keywords internal
xvar_function.rm_mean <- function(xvar, data, grp, digits = 1,
                                  digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                  pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                  percentage = "col") {
  if (!(pvalue | effSize)) show.tests <- FALSE
  ...
  df[, paste0("Full Sample (n=", nrow(data), ")")] <-
    paste0(format(round(mean(x_var, na.rm = TRUE), digits), nsmall = digits),
           " (", format(round(sd(x_var, na.rm = TRUE), digits), nsmall = digits), ")")
  ...
  if (!missing(grp)) {
    ...
    for (level in levels(group_var)) {
      ...
      df[, title] <- mean_by_grp(level, data, xvar, grp, digits)
    }
    ...
    if (pvalue | effSize | show.tests) {
      if (length(unique(no_na_data$group_var)) == 2) {
        ...
        if (show.tests & pvalue) df[, "pTest"] <- "t-test"
        if (pvalue) attr(df, "stat_test") <- "independent t-test"
        if (show.tests & effSize) df[, "effStat"] <- "Cohen's d"
        ...
        if (show.tests & pvalue) df[, "pTest"] <- "ANOVA"
        if (pvalue) attr(df, "stat_test") <- "ANOVA"
        if (show.tests & effSize) df[, "effStat"] <- "omega sq"
      }
    }
  }
...
#' @keywords internal
xvar_function.rm_median <- function(xvar, data, grp, digits = 1,
                                    digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                    pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                    percentage = "col") {
  if (!(pvalue | effSize)) show.tests <- FALSE
  ...
  if (all.stats) {
    ...
    full_col <- paste0("Full Sample (n=", nrow(data), ")")

    # Mean
    df[2, full_col] <- paste0(
      format(round(mean(x_var, na.rm = TRUE), digits), nsmall = digits),
      " (", format(round(sd(x_var, na.rm = TRUE), digits), nsmall = digits), ")"
    )

    # Median with IQR
    x_iqr <- stats::quantile(x_var, na.rm = TRUE, prob = c(0.25, 0.75))
    bracket_iqr <- paste0("(", format(round(x_iqr[1], digits), nsmall = digits),
                          "-", format(round(x_iqr[2], digits), nsmall = digits), ")")
    df[3, full_col] <- paste0(format(round(median(x_var, na.rm = TRUE), digits),
                                     nsmall = digits), " ", bracket_iqr)

    # Range
    x_rng <- range(x_var, na.rm = TRUE)
    bracket_range <- paste0("(", format(round(x_rng[1], digits), nsmall = digits),
                            "-", format(round(x_rng[2], digits), nsmall = digits), ")")
    df[4, full_col] <- bracket_range
    ...
    if (!missing(grp)) {
      ...
      for (level in levels(group_var)) {
        sub <- subset(data, group_var == level)
        title <- paste0(level, " (n=", nrow(sub), ")")
        df[2, title] <- mean_by_grp(level, data, xvar, grp, digits)
        df[3, title] <- median_by_grp(level, data, xvar, grp, TRUE, digits)
        df[4, title] <- median_by_grp(level, data, xvar, grp, FALSE, digits, TRUE)
      }
    }
  } else {
    df <- data.frame(Covariate = xvar)
    ...
    if (iqr) {
      x_iqr <- stats::quantile(x_var, na.rm = TRUE, prob = c(0.25, 0.75))
      bracket <- paste0("(", format(round(x_iqr[1], digits), nsmall = digits),
                        "-", format(round(x_iqr[2], digits), nsmall = digits), ")")
    } else {
      x_rng <- range(x_var, na.rm = TRUE)
      bracket <- paste0("(", format(round(x_rng[1], digits), nsmall = digits),
                        "-", format(round(x_rng[2], digits), nsmall = digits), ")")
    }

    df[, paste0("Full Sample (n=", nrow(data), ")")] <-
      paste0(format(round(median(x_var, na.rm = TRUE), digits), nsmall = digits), " ", bracket)
    ...
    if (!missing(grp)) {
      ...
      for (level in levels(group_var)) {
        sub <- subset(data, group_var == level)
        title <- paste0(level, " (n=", nrow(sub), ")")
        df[, title] <- median_by_grp(level, data, xvar, grp, iqr, digits)
      }
    }
  }

  if (!missing(grp)) {
    no_na_data <- na.omit(data.frame(x_var = x_var, group_var = data[[grp]]))

    if (length(unique(na.omit(data[[grp]]))) < 2 |
        length(unique(no_na_data$group_var)) < 2) {
      df[1, "Missing"] <- sum(is.na(x_var))
      return(df)
    }
    ...
    if (pvalue | effSize | show.tests) {
      if (length(unique(no_na_data$group_var)) == 2) {
        wilcox_test <- wilcox_test_rm(x_var, data[[grp]])
        ...
        if (show.tests & pvalue) df[1, "pTest"] <- "Wilcoxon Rank Sum"
        if (pvalue) attr(df, "stat_test") <- "Wilcoxon rank sum test"
        if (show.tests & effSize) df[1, "effStat"] <- "Wilcoxon r"
      } else if (length(unique(no_na_data$group_var)) > 2) {
        kruskal_test <- kruskal_test_rm(x_var, data[[grp]])
        df[1, "p-value"] <- kruskal_test$p.value
        if (effSize) {
          output <- calc_etaSq_kw(kruskal_test)
          df[1, "Effect Size (95% CI)"] <- format_delta(output)
          attr(df, "eff_size") <- "eta squared"
        }
        if (show.tests & pvalue) df[1, "pTest"] <- "Kruskal Wallis"
        if (pvalue) attr(df, "stat_test") <- "Kruskal Wallis Test"
        if (show.tests & effSize) df[1, "effStat"] <- "eta sq"
      }
    }
  }
...
#' @keywords internal
xvar_function.rm_categorical <- function(xvar, data, grp, digits = 1,
                                         digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                         pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                         percentage = "col") {
  if (!(pvalue | effSize)) show.tests <- FALSE
  ...
  for (xvar_level in levels(x_var)) {
    ...
    if (percentage == "row") {
      ...
      df[i, paste0("Full Sample (n=", nrow(data), ")")] <-
        paste0(nrow(xvar_subset), " (",
               format(round((100 * nrow(xvar_subset) /
                               (nrow(data) - sum(is.na(x_var)))), digits.cat),
                      nsmall = digits.cat), "%)")
    }
    i <- i + 1
  ...
  if (!missing(grp)) {
    ...
    if (pvalue | effSize | show.tests) {
      if (!any(chi_test_rm(cont_table)$expected < 5)) {
        ...
        if (show.tests & pvalue) df[1, "pTest"] <- "Fisher Exact"
        if (pvalue) attr(df, "stat_test") <- "Fisher's exact test"
        if (show.tests & effSize) df[1, "effStat"] <- "Cramer's V"
      }
    }
  }
...
#' Two-level factor summarization
#' @keywords internal
xvar_function.rm_two_level <- function(xvar, data, grp, digits = 1,
                                       digits.cat = 0, iqr = TRUE, all.stats = FALSE,
                                       pvalue = TRUE, effSize = FALSE, show.tests = FALSE,
                                       percentage = "col") {
  if (!(pvalue | effSize)) show.tests <- FALSE
  ...
  if (percentage == "row") {
    ...
    df[, paste0("Full Sample (n=", nrow(temp), ")")] <-
      paste0(sum(x_var, na.rm = TRUE), " (",
             format(round((100 * sum(x_var, na.rm = TRUE) /
                             (nrow(data) - sum(is.na(x_var)))), digits.cat),
                    nsmall = digits.cat), "%)")
  ...
  if (!missing(grp)) {
    ...
    for (grp_level in grp_levels) {
      ...
      title <- paste0(grp_level, " (n=", nrow(sub), ")")
      df[1, title] <- binary_xvar_helper(grp_level, temp, xvar, grp,
                                         digits.cat, percentage)
    }
...
#' Determine variable type for summarization
#' @keywords internal
assign_method <- function(data, xvar, use_mean) {
  if (inherits(data[[xvar]], "factor") &
      length(unique(na.omit(data[[xvar]]))) <= 2) {
    return("rm_two_level")
    ...
    if (identical(use_mean, FALSE)) {
      return("rm_median")
    } else if (is.character(use_mean)) {
      if (xvar %in% use_mean) return("rm_mean") else return("rm_median")
    } else if (is.logical(use_mean)) {
      if (use_mean) return("rm_mean") else return("rm_median")
    }
  }
  stop("xvar must be binary, numeric, or categorical")
}

#' Get digits for specific variable
#' @keywords internal
...
wilcox_test_rm <- function(xvar, grp) {
  ...
  Z <- (U1 + 0.5 - (U1 + U2) / 2) / sqrt((n1 * n2 * (n1 + n2 + 1) / 12))
...
kruskal_test_rm <- function(xvar, grp) {
  kruskal_test <- kruskal.test(xvar~grp)
  n <- length(na.omit(xvar))
  kruskal_test$n <- n
  kruskal_test$xvar <- xvar
  kruskal_test$grp <- grp
  kruskal_test$k <- length(na.omit(unique(grp)))
  class(kruskal_test) <- c(class(kruskal_test), "kruskal_test_rm")
  kruskal_test
}

# Effect Size Calculations with Bootstrap CIs ------------------

#' Calculate Cramer's V with bootstrap CI
#' @keywords internal
calc_CramerV <- function(chisq_test, CIwidth = 0.95) {
  cramer <- sqrt(chisq_test$statistic / (sum(chisq_test$observed) *
                                           (min(dim(chisq_test$observed)) - 1)))
  ...
  bs_cramer <- function(data, indices) {
    dt <- data[indices, ]
    new_chi <- chi_test_rm(table(dt[[1]], dt[[2]]))
    sqrt(new_chi$statistic / (sum(new_chi$observed) *
                                (min(dim(new_chi$observed)) - 1)))
  }

  b_cramer <- boot::boot(df, bs_cramer, R = 1000)
  b_ci <- boot::boot.ci(b_cramer, conf = CIwidth, type = "basic")

  c("cramer v" = cramer, lower = b_ci$basic[4], upper = b_ci$basic[5])
}
...
calc_cohenD <- function(t_test, CIwidth = 0.95) {
  ...
  bs_cohen <- function(data, indices) {
    dt <- data[indices, ]
    new_t <- t_test_rm(dt$xvar, dt$grp)
    abs(new_t$statistic * sqrt((new_t$n[1] + new_t$n[2]) /
                                 (new_t$n[1] * new_t$n[2])))
  }
  ...
  c("cohen d" = cohen, lower = b_ci$basic[4], upper = b_ci$basic[5])
}

#' Calculate Wilcoxon r with bootstrap CI
#' @keywords internal
calc_WilcoxonR <- function(wilcox_test, CIwidth = 0.95) {
  r <- abs(wilcox_test$Z / sqrt(sum(wilcox_test$n)))
  ...
  bs_r <- function(data, indices) {
    ...
    abs(new_wilcox$Z / sqrt(sum(new_wilcox$n)))
  }

  b_r <- boot::boot(df, bs_r, R = 1000)
  b_ci <- boot::boot.ci(b_r, conf = CIwidth, type = "basic")

  c("wilcoxon r" = r, lower = b_ci$basic[4], upper = b_ci$basic[5])
}

#' Calculates eta squared from the KW stat with bootstrap CIs
calc_etaSq_kw <- function(kruskal_test, CI=TRUE){
  eta <- (kruskal_test$statistic-kruskal_test$k+1)/(kruskal_test$n-kruskal_test$k)

  df <- data.frame(xvar = kruskal_test$xvar, grp = kruskal_test$grp)

  bs_eta <- function(data, indices) {
    dt <- data[indices, ]
    new_kruskal <- kruskal_test_rm(dt$xvar, dt$grp)
    (new_kruskal$statistic-new_kruskal$k+1)/(new_kruskal$n-new_kruskal$k)
  }

  b_eta <- boot::boot(df, bs_eta, R = 1000)
  b_ci <- boot::boot.ci(b_eta, conf = CIwidth, type = "basic")

  c("eta sq" = eta, lower = max(0,b_ci$basic[4]), upper = b_ci$basic[5])

}


#' Calculate Omega squared with exact CI from eta squared, or bootstrap CI
#' @keywords internal
calc_omegaSq <- function(anova_test, CIwidth = 0.95,bootstrap=FALSE) {
  summary_anova <- summary(anova_test)
  tbl <- summary_anova[[1]]
  omega <- (tbl[1, 2] - tbl[1, 1] * tbl[2, 3]) /
    (tbl[1, 2] + tbl[2, 2] + tbl[2, 3])

  if (bootstrap){
    # Bootstrap CIs (not recommended)
    bs_omega <- function(data, indices) {
      dt <- data[indices, ]
      new_anova <- stats::update(anova_test, data = dt)
      new_summary <- summary(new_anova)
      tbl <- new_summary[[1]]
      (tbl[1, 2] - tbl[1, 1] * tbl[2, 3]) /
        (tbl[1, 2] + tbl[2, 2] + tbl[2, 3])
    }
    ...
    c("omega squared" = omega, lower = b_ci$basic[4], upper = b_ci$basic[5])

  } else {
    # Exact CIs
    delta_ci <- delta_CI(anova_test)
    eff_ci <- lambda_toAnovaEtaSq(delta_ci,sum(summary_anova[[1]]$Df)+1)
    eff_ci[eff_ci<0] <- 0

    c("omega squared" = omega, lower = eff_ci[1], upper = eff_ci[2])
  }
}

# Transformation functions

# Lambda to effect size transformations
lambda_toAnovaEtaSq <- function(lambda,N){
  lambda/(lambda+N)
}

# Exact CI calculation functions --------
delta_CI <- function(htest, CIwidth=0.95){
  if (inherits(htest,"aov")){
    class(htest) <- c(class(htest),"rm_F")
  } else if (inherits(htest,"htest")){
    if (grepl("Kruskal",htest$method)) {
...
delta_l.rm_chi <- function(htest,CIwidth) {
  ...
  ulim <- 1 - (1-CIwidth)/2
  if (pchisq(chival,df)< ulim) return(0)
...
delta_l.rm_F <- function(htest,CIwidth) {
  ...
  nu2 <- hsum[["Df"]][2]
  ulim <- 1 - (1-CIwidth)/2
  if (pf(Fstat,nu1,nu2) < ulim) return(0)

  lc <- c(.001,Fstat/2,Fstat)
  while(pf(Fstat,nu1,nu2,lc[1])<ulim) {
    lc <- c(lc[1]/4,lc[1],lc[3])
  }
  while(pf(Fstat,nu1,nu2,lc[3])>ulim) {
    lc <- c(lc[1],lc[3],lc[3]+Fstat)
  }
  diff <- 1
  while(diff > .00001) {
    if(pf(Fstat,nu1,nu2,lc[2])<ulim){
      lc <- c(lc[1],(lc[1]+lc[2])/2,lc[2])
    }  else lc <- c(lc[2],(lc[2]+lc[3])/2,lc[3])
    diff <- abs(pf(Fstat,nu1,nu2,lc[2]) - ulim)
  }
  names(lc) <- NULL
  return(lc[2])
}
...
delta_u.rm_chi <- function(htest,CIwidth) {
  alpha <- 1-CIwidth
  chival <- htest$statistic; df <- htest$parameter
  if (pchisq(chival,df)<alpha/2) return(0)
...
delta_u.rm_F <- function(htest,CIwidth) {
  ...
  nu2 <- hsum[["Df"]][2]
  alpha <- 1-CIwidth
  if (pf(Fstat,nu1,nu2)<alpha/2) return(0)

  uc <- c(Fstat,2*Fstat,3*Fstat)
  llim <- (1-CIwidth)/2
  while(pf(Fstat,nu1,nu2,uc[1])<llim) {
    uc <- c(uc[1]/4,uc[1],uc[3])
  }
  while(pf(Fstat,nu1,nu2,uc[3])>llim) {
    uc <- c(uc[1],uc[3],uc[3]+Fstat)
  }
  diff <- 1
  while(diff > .00001) {
    if(pf(Fstat,nu1,nu2,uc[2])<llim)
      uc <- c(uc[1],(uc[1]+uc[2])/2,uc[2])
    else uc <- c(uc[2],(uc[2]+uc[3])/2,uc[3])
    diff <- abs(pf(Fstat,nu1,nu2,uc[2]) - llim)
  }
  names(uc) <- NULL
  return(uc[2])
}
...
format_delta <- function(x, digits = 2) {
  ...
  paste0(out[1], " (", out[2], ", ", out[3], ")")
}

# Group Summary Helper Functions ---------------
...
mean_by_grp <- function(grp_level, data, xvar, grp, digits = 1) {
  ...
  paste0(format(round(mean(new_xvar, na.rm = TRUE), digits), nsmall = digits),
         " (", format(round(sd(new_xvar, na.rm = TRUE), digits), nsmall = digits), ")")
}

#' Calculate median by group
#' @keywords internal
median_by_grp <- function(grp_level, data, xvar, grp, iqr = FALSE,
                          digits = 1, range_only = FALSE) {
  group_var <- data[[grp]]
  subset_grp <- subset(data, group_var == grp_level)
  new_xvar <- subset_grp[[xvar]]
  ...
  paste0(format(round(median(new_xvar, na.rm = TRUE), digits), nsmall = digits),
         " ", bracket)
}

#' Helper for categorical variables by group
#' @keywords internal
categ_xvar_helper <- function(xvar_level, data, xvar, grp, digits.cat = 0,
                              percentage = "col", pct_symbol = TRUE) {
  r_brckt <- if (pct_symbol) "%)" else ")"
  x_var <- data[[xvar]]
  group_var <- data[[grp]]
  ...
  for (grp_level in levels(group_var)) {
    ...
    if (percentage == "row") {
      to_return[2, grp_level] <- paste0(
        nrow(subset_xvar_grp), " (",
        ifelse((nrow(subset_grp) - missing_grp) == 0, 0,
               format(round((100 * nrow(subset_xvar_grp) /
                               (nrow(subset_xvar) - missing_xvar)), digits.cat),
                      nsmall = digits.cat)), r_brckt)
    } else {
      to_return[2, grp_level] <- paste0(
        nrow(subset_xvar_grp), " (",
        ifelse((nrow(subset_grp) - missing_grp) == 0, 0,
               format(round((100 * nrow(subset_xvar_grp) /
                               (nrow(subset_grp) - missing_grp)), digits.cat),
                      nsmall = digits.cat)), r_brckt)
    }
  ...
  if (percentage == "row") {
    ...
    to_return[2, "Full Sample"] <- paste0(
      nrow(subset_xvar), " (",
      format(round((100 * nrow(subset_xvar) /
                      (nrow(data) - missing_xvar)), digits.cat),
             nsmall = digits.cat), r_brckt)
  }
...
#' Helper for binary variables by group
#' @keywords internal
binary_xvar_helper <- function(grp_level, data, xvar, grp, digits.cat = 0,
                               percentage = "col", pct_symbol = TRUE) {
  r_brckt <- if (pct_symbol) "%)" else ")"
  x_var <- data[[xvar]]
  group_var <- data[[grp]]
  ...
  if (percentage == "row") {
    bracket <- paste0(" (", format(round(100 * sum(subset_grp[[xvar]], na.rm = TRUE) /
                                           sum(x_var, na.rm = TRUE), digits.cat),
                                   nsmall = digits.cat), r_brckt)
  } else {
    bracket <- paste0(" (", format(round(100 * sum(subset_grp[[xvar]], na.rm = TRUE) /
                                           (nrow(subset_grp) - num_missing), digits.cat),
                                   nsmall = digits.cat), r_brckt)
  }

  paste0(sum(subset_grp[[xvar]], na.rm = TRUE), bracket)
}
...
#' Generate description of analyses performed
#' @keywords internal
generate_description <- function(xvars, tables) {
  ...
  if (length(names(descr_sum)) > 0) {
    sum_sent <- sapply(names(descr_sum), function(x_sum) {
      paste0(x_sum, " for ", format_strings(descr_sum[[x_sum]]))
    })
    sum_sent <- paste0("Descriptive statistics were calculated as ",
                       format_strings(sum_sent), ".")
    ret <- paste(ret, sum_sent)
  }

  if (length(names(descr_stat)) > 0) {
    stat_sent <- sapply(names(descr_stat), function(x_stat) {
      paste0(x_stat, " for ", format_strings(descr_stat[[x_stat]]))
    })
    stat_sent <- paste0("Between group comparisons were made using ",
                        format_strings(stat_sent), ".")
    ret <- paste(ret, stat_sent)
  }

  if (length(names(descr_eff)) > 0) {
    eff_sent <- sapply(names(descr_eff), function(x_eff) {
      paste0(x_eff, " for ", format_strings(descr_eff[[x_eff]]))
    })
    eff_sent <- paste0("Reported effect sizes are ", format_strings(eff_sent),".")
    print(descr_eff)
    if (any(grepl("omega squared",eff_sent))) eff_sent <- paste(eff_sent,"Exact CIs were computed for omega squared.")
    if (!all(grepl("omega squared",names(descr_eff)))) eff_sent <- paste0(eff_sent,
                                                                         " Bootstrap CIs with 1000 bootstrap samples were calculated for ",
                                                                         format_strings(setdiff(names(descr_eff),"omega squared")),".")
        # eff_sent <- paste0("Reported effect sizes are ", format_strings(eff_sent),
        #                ". 1000 bootstrap samples were used to calculate effect size confidence intervals.")
...
#' Format list of variables for description
#' @keywords internal
